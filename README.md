...
Pick any app below. Point it at a domain you own. Run it.

```bash
git clone https://github.com/supero-platform/supero-apps
cd supero-apps/apps/healthcare/lumen

cp .env.example .env          # then set SUPERO_DOMAIN to a name you pick
./run.sh
```
...
**Access control is enforced by the platform, server-side — declared, not coded.** In
`setup.py` you write policies: row scoping (`filter_field="owner_username"`) and field
hiding (`hidden_fields=["clinical_notes", "diagnosis"]`). The server applies them. So a
patient calling `GET /crud/Appointment` with their own valid token — via curl, bypassing
the UI entirely — **still can't read `clinical_notes`; the field is never serialized.**
That's why `lumen` keeps clinical notes provider-only and `sentinel` hides fraud scores
from claimants without either app writing a line of access-control logic. Open their
`setup.py` — the policy block is short, and it's the thing worth checking first.
...
reachable through the same REST API the app uses, so export is a loop over `GET /crud/...`
