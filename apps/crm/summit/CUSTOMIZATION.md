...
First run creates `.venv`, installs `supero` SDK, runs platform bootstrap (schemas, seed data, policies), then starts the server at `http://localhost:5669`.
...
Container image is ~200MB (python:3.11-slim base), runs as non-root user, includes healthcheck on port 5669.
...
```javascript
theme: 'fintech',   // other options: health, education, saas, retail, food, legal, fitness, generic
```
...
dashboardWidgets: [
    ...
    { label: 'Active Items', schema: 'item', aggregate: 'count', icon: '🐕' },
],
```
...
const TABS = [
    {
        ...
        label: 'Items',
        icon: '🐕',
        schema: 'item',                  // matches schemas.py entry
        attrs: [
            { name: 'item_name', type: 'string', mandatory: true, featured: true },
...
cardTemplate: {
    ...
    title: 'item_name',              // most prominent field (required)
    subtitle: 'breed',
...
introSection: {
    title: 'Why PawSpa',
    description: 'What makes our service different.',
    features: [
...
SUPERO_TENANT_NOUN_PLURAL=Salons
PORT=5669                                    # local web server port
```
