# Concierge AI — AI assistant context

> Read this before editing. It describes **this app specifically**.

## What this app is

AI-first customer support — deflect with a grounded AI concierge, escalate to humans with full context.

- **Core entities** (`schemas.py`): `Article`, `Ticket`, `Message`, `Macro`
- **Services on** (`config.py`): `ai`, `email`, `slack`, `workflows`
- **Runs at** `http://localhost:5662` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~600 lines of `React.createElement`)
