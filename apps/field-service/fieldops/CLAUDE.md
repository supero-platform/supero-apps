...
- **Core entities** (`schemas.py`): `Service`, `Quote`, `QuoteStep`, `Appointment`, `WorkOrder`, `WorkOrderSignature`, `Payment`, `PartItem`, `PartHold`, `Contract`, `CustomerProfile`, `Technician`
- **Services on** (`config.py`): `email`, `sms`, `stripe_checkout`, `quickbooks`, `google_calendar`, `slack`, `ai`, `service`, `approval`, `appointment`, `document_signature`, `payment`, `inventory`, `recurring_plan`, `workflows`
- **Runs at** `http://localhost:5651` after `./run.sh`
