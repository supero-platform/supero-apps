...
First run creates `.venv`, installs `supero` SDK, runs platform bootstrap (schemas, seed data, policies), then starts the server at `http://localhost:5651`.
...
Container image is ~200MB (python:3.11-slim base), runs as non-root user, includes healthcheck on port 5651.
...
PORT=5651                                    # local web server port
```
