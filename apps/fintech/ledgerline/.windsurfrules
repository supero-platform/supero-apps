# Ledgerline — AI assistant context
...
- **Core entities** (`schemas.py`): `Plan`, `Customer`, `Invoice`, `Expense`
- **Services on** (`config.py`): `stripe_checkout`, `email`, `workflows`
- **Runs at** `http://localhost:5664` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~400 lines of `React.createElement`)
