# BrightSmile — AI assistant context

> Read this before editing. It describes **this app specifically**.

## What this app is

Modern dental care, close to home — book online across our neighborhood clinics, meet your dentist, and keep clinical notes care-team-only.

- **Core entities** (`schemas.py`): `Location`, `Dentist`, `DentalService`, `Patient`, `Appointment`, `Treatment`
- **Services on** (`config.py`): `email`, `sms`, `ai`, `workflows`
- **Runs at** `http://localhost:5667` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~650 lines of `React.createElement`)
