# Lumen Health — AI assistant context
...
- **Core entities** (`schemas.py`): `Provider`, `ClinicService`, `Patient`, `Appointment`, `Document`
- **Services on** (`config.py`): `email`, `sms`, `ai`, `workflows`, `google_calendar`
- **Runs at** `http://localhost:5663` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~550 lines of `React.createElement`)
