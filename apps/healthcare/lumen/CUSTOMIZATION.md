...
First run creates `.venv`, installs `supero` SDK, runs platform bootstrap (schemas, seed data, policies), then starts the server at `http://localhost:5663`.
...
Container image is ~200MB (python:3.11-slim base), runs as non-root user, includes healthcheck on port 5663.
...
SUPERO_TENANT_NOUN_PLURAL=Salons
PORT=5663                                    # local web server port
```
