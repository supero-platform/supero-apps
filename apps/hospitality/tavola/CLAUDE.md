# Tavola — AI assistant context

> Read this before editing. It describes **this app specifically**.

## What this app is

Your table, your order, your way — online ordering, table reservations and loyalty across the Tavola restaurant group.

- **Core entities** (`schemas.py`): `Restaurant`, `MenuItem`, `Order`, `OrderLine`, `Reservation`
- **Services on** (`config.py`): `email`, `sms`, `stripe_checkout`, `workflows`
- **Runs at** `http://localhost:5673` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~1000 lines of `React.createElement`)
