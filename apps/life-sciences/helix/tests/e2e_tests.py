...
SERVER_URL = f"http://localhost:{_env('PORT', _env('UI_PORT', '5648'))}"
...
def test_connectivity(S):
    ...
    # validated by test_bundle.py at extraction time.
    static_probes = [
...
def main():
    ap = argparse.ArgumentParser(
        ...
        epilog="For pre-flight validation (no server): python3 test_bundle.py\n\nSections: " + ", ".join(ALL))
    ...
    args = ap.parse_args()
    if args.list: print("Sections:"); [print(f"  {s}") for s in ALL]; print("\nPre-flight: python3 test_bundle.py"); sys.exit(0)
