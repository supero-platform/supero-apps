# TrialCore — AI assistant context
...
- **Core entities** (`schemas.py`): `Trial`, `Site`, `Participant`, `Visit`, `AdverseEvent`, `TrialDocument`
- **Services on** (`config.py`): `email`, `slack`, `ai`, `workflows`
- **Runs at** `http://localhost:5668` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~900 lines of `React.createElement`)
