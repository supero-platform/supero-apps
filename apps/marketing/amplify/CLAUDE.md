# Amplify — AI assistant context

> Read this before editing. It describes **this app specifically**.

## What this app is

Launch campaigns everywhere, from one place — connect every social channel, compose with AI, schedule, publish and track performance.

- **Core entities** (`schemas.py`): `Plan`, `Workspace`, `Channel`, `Campaign`, `Post`
- **Services on** (`config.py`): `email`, `slack`, `ai`, `workflows`, `instagram`, `linkedin`, `x_social`, `youtube`
- **Runs at** `http://localhost:5672` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~850 lines of `React.createElement`)
