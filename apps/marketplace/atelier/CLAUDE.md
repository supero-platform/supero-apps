...
- **Core entities** (`schemas.py`): `Brand`, `Product`, `Buyer`, `Order`, `OrderItem`
- **Services on** (`config.py`): `email`, `stripe_checkout`, `ai`, `workflows`
- **Runs at** `http://localhost:5661` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~1050 lines of `React.createElement`)
