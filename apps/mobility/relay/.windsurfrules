...
- **Core entities** (`schemas.py`): `Shift`, `Credential`, `Verification`, `VerificationStep`, `Timesheet`, `TimesheetStep`, `Payout`, `Contract`, `Profile`
- **Services on** (`config.py`): `email`, `sms`, `push_notification`, `google_calendar`, `ai`, `stripe_checkout`, `booking`, `attachment`, `approval`, `payment`, `recurring_plan`, `workflows`
- **Runs at** `http://localhost:5666` after `./run.sh`

## The UI pattern — read this first

This app has a **bespoke full-page UI**. `ui/app.js` (~700 lines of `React.createElement`)
